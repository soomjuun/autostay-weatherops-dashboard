...
v2.16.4 최종 교차검증 hotfix 기준으로 대시보드 상단·지점 상세의 상태값은 `Red` 같은 영문 코드가 아니라 `제한확인` 등 운영 라벨로 표시합니다. Apps Script 업스트림 응답에는 10초 타임아웃과 비JSON 응답 가드를 적용해, 장애 시 샘플이나 깨진 화면이 아니라 구조화된 오류 배너가 노출되도록 했습니다.
...
| `WEATHER_OPS_UPSTREAM_TIMEOUT_MS` | Apps Script 요청 1회 응답 제한시간. 기본값 `50000`. |
| `WEATHER_OPS_UPSTREAM_RETRY_COUNT` | 일시적 timeout·네트워크 실패 재시도 횟수. 기본값 `0`. Apps Script의 긴 cold 응답을 중간에 끊지 않고 Vercel 함수 제한 60초 안에서 한 번 완료합니다. |
...
- `visuals.recoveryFunnel`의 AS 차단 항목은 `key: "asBlocked"` 또는 라벨 `AS 차단`으로 전달합니다. 대시보드는 이 항목을 CRM·재방문 전환율 계산에서 제외하고 현재 AS 차단과 과거 집계 이력을 분리합니다. 단계 건수가 역전되면 서로 다른 집계 단위로 보고 전환율을 계산하지 않습니다.
- `marketingActions`에서 지점·제안·담당·상태·우선순위가 같은 행은 화면에서 한 건으로 합치고 가장 이른 기한과 동일 제안 반복 횟수를 표시합니다. 원본 원장 행과 이력은 변경하지 않습니다.
...
- heat 트리거는 공식 폭염특보와 내부 최고기온 임계값을 함께 포괄하는 `고온/폭염`으로 표시합니다. prod 기상 신호는 운영 액션 원장이나 shadow 검증 신호와 합치지 않습니다.
- 현장 취약정보는 운영 위험등급, 운영 제한, 관리자 입력 또는 별도 workflow task를 자동 변경하지 않습니다. 현재 Apps Script build 계약은 `2026-08-25-revenue-runtime-hotfix.20`입니다.
- `.17`부터 `.20`까지는 매출 파이프라인 무결성, 중복 조치 통합, 적재 검증, 계산 성능을 개선했으며 dashboard payload 버전과 지점, 기상, 액션 필드 계약은 `v2.16.4-weather-signal.2`로 유지합니다. 대시보드는 기존 필드를 그대로 자동 반영합니다.
...
- 운영 화면은 5분마다 실데이터를 자동 갱신하고, 오래 비활성화한 탭으로 돌아오면 Apps Script 캐시를 우회해 즉시 다시 조회합니다.
- 최신 기상 신호가 없을 때는 prod 운영 원장 상태와 기상 판단 대기를 문구로 명확히 분리합니다.
