...
- 오늘 지점 상태: 필터 기준 지점 상태를 심각도 순 누적 막대와 우선 확인 지점 리스트로 표시
...
- 운영 시각화 레이아웃: `오늘 지점 상태`가 좌측 2행을 차지하고, `오늘 기상 리스크` 아래에 `지점별 회복 진행`, `회복 실행 단계` 아래에 `이용 회복 vs 매출 회복`이 붙도록 명시 영역 배치를 사용
