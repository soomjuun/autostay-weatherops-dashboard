...
test('상태 필터와 정적 자산 버전이 배포용 표기를 사용한다', () => {
  ...
  assert.match(html, /app\.js\?v=2026-07-16-5/);
  assert.match(html, /style\.css\?v=2026-07-16-5/);
...
test('지점 원천 오류를 위험등급과 분리해 확인 대상으로 표시한다', () => {
  ...
  assert.equal(api.signalSourceNotice(store), '원천 확인');
  assert.equal(api.signalSourceDetail(store), '에어코리아 측정값 없음');
});
