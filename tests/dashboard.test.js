...
test('1899년 스프레드시트 기준일은 피크 시간으로 노출하지 않는다', () => {
  ...
  assert.equal(api.formatActionDue('Sat Dec 30 1899 10:19:00 GMT+0827 (Korean Standard Time)'), '10:19');
});

test('우선 확인 큐는 공식 액션 지점을 이름으로 매칭해 중복 집계하지 않는다', () => {
  const { api } = loadDashboardLogic();
  api.state.risk = 'all';
  api.state.store = 'all';
  api.state.data = {
    stores: [
      { id: 'hanam', name: '하남 미사', status: 'Orange', prodStatus: 'Orange', signalRiskType: '강수·폭염', nextAction: '현장 확인' },
      { id: 'jayuro', name: '자유로 88', status: 'Orange', prodStatus: 'Orange', signalRiskType: '강수·폭염', nextAction: '현장 확인' },
      { id: 'seongsu', name: '서울 성수', status: 'Orange', prodStatus: 'Orange', signalRiskType: '폭염', nextAction: '현장 확인' },
      { id: 'goyang', name: '고양 삼송', status: 'Orange', prodStatus: 'Green', signalRiskType: '강수·폭염', nextAction: '기상 확인' },
      { id: 'gwangmyeong', name: '광명점', status: 'Orange', prodStatus: 'Green', signalRiskType: '강수·폭염', nextAction: '기상 확인' },
      { id: 'anseong', name: '안성 석정', status: 'Orange', prodStatus: 'Green', signalRiskType: '폭염', nextAction: '기상 확인' },
      { id: 'ilsan', name: '일산 풍동', status: 'Yellow', prodStatus: 'Green', signalRiskType: '강수·폭염', nextAction: '기상 확인' }
    ],
    opsActions: [
      { store: '하남 미사', level: 'Orange', riskTypes: ['rain', 'heat'], owner: '박준영 매니저', due: '10:19', action: '현장 확인' },
      { store: '자유로 88', level: 'Orange', riskType: 'rain,heat', owner: '정진호 매니저', due: '10:20', action: '현장 확인' },
      { store: '서울 성수', level: 'Orange', riskType: 'heat', owner: '한석태 매니저', due: '10:21', action: '현장 확인' }
    ]
  };
...
test('heat 트리거와 활성 기상 신호를 고온/폭염으로 일관되게 표시한다', () => {
  ...
  assert.equal(api.riskColumnLabel({ key: 'heat', label: '폭염' }), '고온/폭염');
...
test('API 프록시는 스프레드시트 시간값에서 1899년 기준일을 제거한다', async () => {
  const handlerPath = path.join(ROOT, 'api', 'weather-ops-data.js');
  const originalFetch = global.fetch;
  const originalApiUrl = process.env.WEATHER_OPS_API_URL;
  const originalAllowSample = process.env.WEATHER_OPS_ALLOW_SAMPLE;
  try {
    process.env.WEATHER_OPS_API_URL = 'https://example.invalid/weather-ops';
    process.env.WEATHER_OPS_ALLOW_SAMPLE = 'false';
    global.fetch = async () => ({
      ok: true,
      status: 200,
      text: async () => JSON.stringify({
        version: 'v2.16.4',
        generatedAt: '2026-07-24T11:21:30+09:00',
        summary: {},
        stores: [],
        opsActions: [{
          store: '하남 미사',
          due: '10:19',
          dueAt: '1899-12-30T10:19:00+08:27',
          nextUpdateDue: 'Sat Dec 30 1899 10:19:00 GMT+0827 (Korean Standard Time)'
        }]
      })
    });
    delete require.cache[require.resolve(handlerPath)];
    const handler = require(handlerPath);
    const res = mockResponse();
    await handler({ method: 'GET', url: '/api/weather-ops-data?fresh=1' }, res);
    assert.equal(res.statusCode, 200);
    assert.equal(res.body.opsActions[0].dueAt, '10:19');
    assert.equal(res.body.opsActions[0].nextUpdateDue, '10:19');
    assert.doesNotMatch(JSON.stringify(res.body.opsActions[0]), /1899/);
  } finally {
    global.fetch = originalFetch;
    if (originalApiUrl === undefined) delete process.env.WEATHER_OPS_API_URL;
    else process.env.WEATHER_OPS_API_URL = originalApiUrl;
    if (originalAllowSample === undefined) delete process.env.WEATHER_OPS_ALLOW_SAMPLE;
    else process.env.WEATHER_OPS_ALLOW_SAMPLE = originalAllowSample;
    delete require.cache[require.resolve(handlerPath)];
  }
});
...
test('최신 payload와 build 식별자를 프런트 계약에 보존한다', () => {
  ...
  assert.equal(normalized.buildId, '2026-08-25-revenue-runtime-hotfix.20');
...
test('최신 현장 취약정보 계약을 운영 지점과 기상 신호 양쪽에서 보존한다', () => {
  ...
  const normalized = api.normalize({
    version: 'v2.16.4',
    dashboardPayloadVersion: 'v2.16.4-weather-signal.2',
    system: { scriptBuildId: '2026-08-25-revenue-runtime-hotfix.20' },
    stores: [{
      storeId: 'hanam',
      storeName: '하남 미사',
      prodStatus: 'Green',
...
test('기대 버전은 환경변수 설정 시에만 고정 비교한다', () => {
  ...
  assert.match(proxySource, /2026-08-25-revenue-runtime-hotfix\.20/);
